...
func TestCanonicalGatewayPayloadFixtures(t *testing.T) {
	...
	for _, fixture := range loadGatewayFixtures(t) {
		t.Run(fixture.Name, func(t *testing.T) {
			...
			if !verifyGatewaySignature(canonical, fixture.GatewaySignature) {
			...
			if code, msg := validateGatewayEnvelope(fixture.Payload); code != "" || msg != "" {
				t.Fatalf("validateGatewayEnvelope mismatch: %s %s", code, msg)
			}
		})
	}
}
