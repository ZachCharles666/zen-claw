...
def test_4eyes_principle():
    ...
    response = client.post(
        "/v1/skills/skill-2/1.0/review",
        json={"approved": True, "notes": "ok"},
        headers={"x-api-token": "pub_token"},
    )
...
def test_tenant_isolation():
    ...
    response = client.post(
        "/v1/skills/skill-3/1.0/review",
        json={"approved": True, "notes": "no"},
...
def test_takedown():
    ...
    response = client.post(
        "/v1/skills/skill-4/takedown?reason=DMCA", headers={"x-api-token": "pub_token"}
    )
    assert response.status_code == 403

    # Security admin can takedown
    response = client.post(
        "/v1/skills/skill-4/takedown?reason=DMCA", headers={"x-api-token": "sec_token"}
    )
    assert response.status_code == 200
