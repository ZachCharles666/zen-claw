...
def check_venv():
    if sys.prefix == sys.base_prefix:
        ...
        print(
            "    Please run `.\\.venv\\Scripts\\Activate.ps1` (Windows) or `source .venv/bin/activate` (Mac/Linux) first.\n"
        )
        sys.exit(1)
...
def simulate_webhook_traffic():
    ...
    try:
        ...
        try:
            ...
            if status_code == 403:
                print(
                    "[+] Security Guard works! Unauthorized webhook blocked with 403 as expected."
                )
            else:
...
def main():
    ...
    steps = [test_cli_basics, test_agent_interaction, simulate_webhook_traffic]
