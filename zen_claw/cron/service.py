...
from zen_claw.cron.types import CronJob, CronJobState, CronPayload, CronSchedule, CronStore
...
class CronService:
            ...
            "jobs": [
                {
                    ...
                    "payload": {
                        ...
                        "to": j.payload.to,
                        "targetUrl": j.payload.target_url,
    ...
    async def _execute_job(self, job: CronJob) -> None:
        ...
        logger.info(
            f"Cron: executing job '{job.name}' ({job.id}) "
            + TraceContext.event_text("cron.job.start", trace_id, job_id=job.id)
        )
        ...
        try:
            ...
            logger.info(
                ...
                + TraceContext.event_text("cron.job.done", trace_id, job_id=job.id)
            )
