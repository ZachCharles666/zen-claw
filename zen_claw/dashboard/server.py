...
def build_dashboard_snapshot(config: "Config") -> dict[str, Any]:
    ...
    from zen_claw.observability.audit import verify_audit_log, verify_security_replay_consistency
    ...
    replay_verify = verify_security_replay_consistency(audit_log_path)
    ...
    return {
        ...
        "security": {
            ...
            "local_identity_channels": list(config.tools.policy.local_identity_channels or []),
            "dev_profiles": [
            ...
            "replay_consistency_ok": bool(replay_verify.get("ok")),
            "replay_consistency_checked": int(replay_verify.get("checked") or 0),
            "replay_consistency_groups": int(replay_verify.get("groups") or 0),
            "replay_consistency_error": str(replay_verify.get("error") or ""),
            "replay_consistency_mismatches": list(replay_verify.get("mismatches") or [])[:10],
        },
...
<button type="submit">登录</button></form></div></body></html>"""
    ...
    @api_router.get(
        "/security/audit/replay-verify",
        summary="Security Audit Replay Verify",
        description="Verify cross-component replay consistency across approval, tool, and outbound security audit events.",
        tags=["dashboard"],
    )
    async def api_security_audit_replay_verify():
        from zen_claw.config.loader import get_data_dir
        from zen_claw.observability.audit import verify_security_replay_consistency

        audit_log_path = (
            get_data_dir() / "audit_logs" / f"{datetime.now(tz=UTC).strftime('%Y-%m-%d')}.jsonl"
        )
        return JSONResponse(verify_security_replay_consistency(audit_log_path))
